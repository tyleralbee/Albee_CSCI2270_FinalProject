...
void HashTable::buyItem(string name, int offer){
	int index = hashSum(name,tableSize);
	bool found = false;

	if (hashTable[index] != NULL){
		for (int i = 0; i < hashTable[index]->size(); i++){
			if ((*hashTable[index])[i].item == name && (*hashTable[index])[i].price <= offer){
				hashTable[index]->erase(hashTable[index]->begin() + i);
				cout<<name<<" has been sold."<<endl;
				found = true;
				break;
			}
			else if((*hashTable[index])[i].item == name && (*hashTable[index])[i].price > offer){
				cout<<"There is no "<<name<<" available for that price"<<endl;
				found = true;
